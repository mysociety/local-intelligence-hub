...
export const currentOrganisationIdAtom = atomWithStorage<string>('mappedOrganisationId', "", {
    ...
    subscribe: typeof window !== 'undefined' && 'cookieStore' in window ? (key, callback, initialValue) => {
        const storageEventCallback = (e: any) => {
            const { changed, deleted, type } = e;

            if (type === 'change') {
                const { value: newTokenCookieValue } = changed.find((i: any) => i.name === key) ?? {};
