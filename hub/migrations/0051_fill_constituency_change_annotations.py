# Generated by Django 4.1.2 on 2023-06-15 10:50
...
def update_fields(apps, schemaeditor):  # pragma: no cover
...
def reverse(apps, schema_editor):  # pragma: no cover
    ...
    DataSet.objects.filter(
        name__in=constituency_change_annotations_lookup.keys()
    ).update(unit_type=None)
    DataSet.objects.filter(
        name__in=constituency_change_annotations_lookup.keys()
    ).update(unit_distribution=None)


class Migration(migrations.Migration):

    dependencies = [
        ("hub", "0050_add_constituency_change_annotation_fields"),
    ]

    operations = [migrations.RunPython(update_fields, reverse_code=reverse)]
