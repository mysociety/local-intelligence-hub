...
from hub.parsons.etl import Table
from hub.parsons.utilities import check_env
from hub.parsons.utilities.api_connector import APIConnector
...
class RateLimitedAPIConnector(APIConnector):
    ...
    def __init__(self, *args, rate_limit: str, rate_limit_id: str, backoff_seconds: float = 1, **kwargs):
        self.rate_limit_id = rate_limit_id
        self.backoff_seconds = backoff_seconds
        self.rate = self.allowed_requests_per_second(rate_limit)
        super().__init__(*args, **kwargs)

    # Normal self.request, but handling the 4/s ratelimit
    def request(self, *args, **kwargs):
        ...
        if len(history) < 4:
            return req()
        # Remove the oldest history item if more than 4, to conserve memory
        self.trim_request_history()
        # Check request rate via history
        if (history[0] - history[-1]).seconds < self.rate:
    ...
    @property
    def request_history_id(self):
        return f"an_rate_limit_{hash(self.rate_limit_id)}"

    @property
    def request_history(self) -> list[datetime]:
        return cache.get(self.request_history_id, [])

    def add_request_to_history(self):
        self.request_history.insert(0, datetime.now())
        
    def trim_request_history(self):
        return cache.set(self.request_history_id, self.request_history[:4])

    def reset_request_history(self):
        return cache.set(self.request_history_id, [])
    ...
    def allowed_requests_per_second(self, rate: str):
        rate_tuple = self.parse_rate(rate)
        return rate_tuple[0] / rate_tuple[1]
