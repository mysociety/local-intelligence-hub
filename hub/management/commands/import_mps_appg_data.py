...
class Command(BaseCommand):
    ...
    def add_mps(self, df):
        ...
        df["mp"] = df.twfy_id.dropna().apply(lambda x: twfy_ids.filter(data=x).first().person if twfy_ids.filter(data=x) else None)
        df = df.dropna(subset='mp')
        return df

    def get_df(self):
        # TODO: Change this when APPG dataset it published.
    ...
    def import_results(self):
        ...
        PersonData.objects.filter(data_type=data_type).delete()
