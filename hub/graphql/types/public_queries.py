...
) -> List[model_types.GenericData]:
    user = get_current_user(info)
    external_data_source: models.ExternalDataSource = models.ExternalDataSource.objects.get(
        pk=external_data_source_id
    )
    permissions = models.ExternalDataSource.user_permissions(user, external_data_source)
    if not permissions.get("can_display_points") or not permissions.get("can_display_details"):
